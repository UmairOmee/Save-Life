...
class UserPendingRequests extends React.Component{
    ...
    render(){
        ...
        // const filterArrayCompleted = filterArray.filter(item => item.status === "completed");
        // const filterByType = filterArrayCompleted.filter(item => item.type === "map" || "form")
        ...
        return(
            <DashboardSidebar>
                <div>
                <h1 className="display-4 text-center">User Completed Requets</h1>
...
const mapStateToProps = (store, props) => ({
    user: store.user,
    bookings: store.bookings.bookings.filter(item => item.status === "completed"),
})
